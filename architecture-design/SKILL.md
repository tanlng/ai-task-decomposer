...
description: 系统架构设计与演化。先把新需求分流为补丁或 MVP 演化，再用 MVP 作为最高筛选器，把想法收敛为可迭代交付的架构方案；按能力模块推进 PRD、SDD、按需 DDD、显式 Iteration Plan、稳定/临时节点管理与 Implementation Readiness。适用于项目早期方案设计、架构改造、职责重划、跨模块链路调整、外部系统接入，以及需要多轮协作的大方案。
---
...
- 需要先澄清产品行为、交互规则、协议语义或验收标准时，先联动 `prd-driven-development`，不要跳过 PRD 直接做架构。
